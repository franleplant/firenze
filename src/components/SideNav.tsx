		...
		<>
			<nav className='sm:w-1/5 lg:w-1/6 min-w-max'>
				<section className='sticky sm:hidden bg-gray-600 text-white p-3 top-0'>
					<button onClick={() => setOpenSideBar(!openSideBar)} className='text-white text-xl sm:hidden z-50'>
						+
					</button>
				</section>
				<section
					className={`asdasdasd ${
						openSideBar ? " flex h-screen sm:h-auto fixed z-40 w-full flex-row" : " hidden "
